...
export class MailService {
  ...
  async sendAppointmentEmail(visitorEmail: string, date: string, allocatedTime: string, formLink: string) {
    const ndaFormLink = `https://192.168.1.57:3001/nda-form.html?email=${encodeURIComponent(visitorEmail)}&date=${encodeURIComponent(date)}&time=${encodeURIComponent(allocatedTime)}`;
    const safetyVideoLink = `https://192.168.1.57:3001/safety-video.html?email=${encodeURIComponent(visitorEmail)}&date=${encodeURIComponent(date)}&time=${encodeURIComponent(allocatedTime)}`;
    const mailOptions = {
